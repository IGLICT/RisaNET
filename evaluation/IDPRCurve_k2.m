function [ st,prv,retrievalindex ] = IDPRCurve_k2( idmatname, labelname ,method)
...
modelnum = size(labelstruct.label,1);
...
for i = 1 : modelnum
    ...
    macro_map(labelinfo(i)) = macro_map(labelinfo(i)) + mean(pr{i}.precision);
    ft= ft+pr{i}.firsttier;
    macro_ft(labelinfo(i)) = macro_ft(labelinfo(i)) + pr{i}.firsttier;
    st = st + pr{i}.secondtier;
    macro_st(labelinfo(i)) = macro_st(labelinfo(i)) + pr{i}.secondtier;
    ndcg = ndcg + pr{i}.ndcg;
    macro_ndcg(labelinfo(i)) = macro_ndcg(labelinfo(i)) + pr{i}.ndcg;
    nearest = nearest + pr{i}.nearest;
    macro_nearest(labelinfo(i)) = macro_nearest(labelinfo(i)) + pr{i}.nearest;
%     dcg = dcg + pr{i}.dcg;    
end
...
color8 = sscanf(str8(2:end),'%2x%2x%2x',[1 3])/255;
hold on;
...
if strcmp(method, 'h') == 1
    plot(st,prv,'Color', color8,'LineWidth',4);
end
legend('w/ scale + VAE','w/ scale + CNN', 'w/o scale + VAE', 'w/ scale + CNN')
